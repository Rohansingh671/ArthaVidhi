				...
				<div class="page-header">
					<div class="add-item d-flex">
						<div class="page-title">
							<h4>Labour Code Details</h4>
							<h6>Manage your labour code</h6>
						</div>
					</div>
					...
					<div class="page-btn">
						<a href="#" class="btn btn-added" data-bs-toggle="modal" data-bs-target="#add-brand"><i data-feather="plus-circle" class="me-2"></i>Add New LabourCode</a>
					</div>
				</div>
				<!-- /product list -->
						...
						<div class="table-responsive">
							<table class="table  datanew">
								...
								<tbody>
									...
									require_once 'php/labourDataFromDatabase.php';
									foreach ($labourCodes as $labourCode) {
									?>
										<tr>
											<td><a href="#" class="link-primary"><?php echo $labourCode['labourID']; ?></a></td>
											<td><?php echo $labourCode['labourCode']; ?></td>
											<td><?php echo $labourCode['labourCharge']; ?></td>
											<td><?php echo $labourCode['status']; ?></td>
				...
				<div class="page-wrapper-new p-0">
					<div class="content">
						...
						<div class="modal-body custom-modal-body new-employee-field">
							<form action="php/labourCode.php" method="post">
								<?php
								require_once 'php/databaseConnection.php';

								$mysqli = db_connect();

								$result = $mysqli->query("SELECT labourID FROM labourCode ORDER BY ID DESC LIMIT 1");
								$row = $result->fetch_row();

								if ($row == null) {
									$total_code_id = 1; // Initialize $total_group_id here for the case where no rows are found
								} else {
									$code_id = substr($row[0], 3); // Adjust substring length to skip "FGID"
									$total_code_id = $code_id + 1; // Increment the numeric part of the group ID
								}
								?>
								<div class="mb-3">
									<label class="form-label">Labour ID</label>
									<input type="text" name="labourID" class="form-control" value="LAB<?php echo $total_code_id ?>" readonly>
								</div>
								<div class="mb-3">
									<label class="form-label">Labour Code</label>
									<input type="text" name="labourCode" class="form-control">
								</div>
								<div class="mb-3">
									<label class="form-label">Labour Charge</label>
									<input type="text" name="labourCharge" class="form-control">
								</div>
								<div class="mb-0">
									<label class="form-label">Status</label>
									<select class="select" name="codeStatus">
								...
								<div class="modal-footer-btn">
									...
									<button type="submit" class="btn btn-submit">Save LabourCode</button>
								</div>
							</form>
						</div>
					</div>
